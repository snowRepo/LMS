...
try {
    ...
    if (!$librarian) {
        header('Location: librarians.php?error=Librarian not found or account is not pending');
        exit;
    }
    ...
    $pageTitle = 'Resend Librarian Setup Email - ' . $librarian['first_name'] . ' ' . $librarian['last_name'];
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resend_setup'])) {
    try {
        ...
        $result = $stmt->execute([$verificationToken, $librarianId]);
        ...
        if ($result) {
            ...
            $emailSent = $emailService->sendLibrarianSetupEmail($librarian['email'], $emailData);
            
            if ($emailSent) {
                header('Location: view_librarian.php?id=' . urlencode($librarianId) . '&success=' . urlencode('Setup email resent successfully to ' . $librarian['email']));
                exit;
            } else {
                header('Location: resend_librarian_setup.php?id=' . urlencode($librarianId) . '&error=' . urlencode('Failed to send setup email. Please try again.'));
                exit;
            }
        } else {
            header('Location: resend_librarian_setup.php?id=' . urlencode($librarianId) . '&error=' . urlencode('Failed to update user. Please try again.'));
            exit;
        }
    } catch (Exception $e) {
        header('Location: resend_librarian_setup.php?id=' . urlencode($librarianId) . '&error=' . urlencode('Error resending setup email: ' . $e->getMessage()));
        exit;
    }
}
?>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-paper-plane"></i> Resend Librarian Setup Email</h1>
            <p>Resend setup email to <?php echo htmlspecialchars($librarian['first_name'] . ' ' . $librarian['last_name']); ?></p>
        </div>
        ...
        <div class="content-card">
            ...
            <div class="warning-message">
                <h3><i class="fas fa-info-circle"></i> Resend Setup Email Information</h3>
                <p>You are about to resend a setup email to <strong><?php echo htmlspecialchars($librarian['email']); ?></strong>.</p>
                <p>The librarian will receive an email with instructions to set up their account. The setup link will expire in 24 hours.</p>
            </div>
            ...
            <form method="POST" id="resendForm">
                <div class="action-buttons">
                    <button type="button" name="resend_setup" class="btn btn-warning" id="resendButton">
                ...
                <input type="hidden" name="resend_setup" value="1">
            </form>
        </div>
