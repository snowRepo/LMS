...
try {
    ...
    if ($recipientType === 'all_librarians') {
        ...
        $stmt = $db->prepare("
            ...
            WHERE user_id = ? AND library_id = ? AND status = 'active'
        ");
        ...
        if ($senderRole === 'supervisor' && $recipientRole === 'librarian') {
            $canMessage = true;
        } else if ($senderRole === 'librarian' && in_array($recipientRole, ['supervisor', 'member'])) {
            $canMessage = true;
        } else if ($senderRole === 'member' && $recipientRole === 'librarian') {
            $canMessage = true;
        }
        
        if (!$canMessage) {
            header('Content-Type: application/json');
            echo json_encode(['error' => 'You are not allowed to message this user']);
            exit;
        }
