...
if (isset($_GET['token']) && !empty($_GET['token'])) {
    ...
    try {
        ...
        // Find member with this verification token and pending status
        $stmt = $db->prepare("
        ...
        if ($user) {
            ...
            if ($tokenAge > 86400) {
                $message = 'Setup link has expired. Please contact your library to resend the setup email.';
                $messageType = 'error';
            } else {
            ...
            $message = 'Invalid or expired setup link. Please check the link or contact your library.';
            $messageType = 'error';
        }
    ...
    try {
        ...
        if (empty($password) || empty($confirmPassword)) {
            ...
            if ($result && $stmt->rowCount() > 0) {
                // Get library ID and user's integer ID for activity log
                $stmt = $db->prepare("SELECT id, library_id FROM users WHERE user_id = ?");
                $stmt->execute([$userId]);
                $userData = $stmt->fetch();
                $userIntegerId = $userData['id'];
                $libraryId = $userData['library_id'];
                
                // Log the setup activity
                $stmt = $db->prepare("
                    INSERT INTO activity_logs 
                    (user_id, library_id, action, entity_type, entity_id, description, ip_address, user_agent, created_at)
                    VALUES (?, ?, 'member_setup', 'user', ?, 'Member account setup completed successfully', ?, ?, NOW())
                ");
                $stmt->execute([
                    $userIntegerId,
                    $libraryId,
...
$pageTitle = 'Member Account Setup';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .success-content .message-container {
            width: 100%;
            max-width: 100%;
            box-sizing: border-box;
        }
        ...
        <div class="setup-card">
            ...
            <?php elseif ($messageType === 'success'): ?>
                ...
                <div class="success-content">
                    ...
                    <a href="login.php" class="btn btn-primary" style="margin-top: 1.5rem;">
