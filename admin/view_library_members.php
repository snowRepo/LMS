...
if ($libraryId <= 0) {
    header('Location: member_reports.php');
    exit;
}

// Handle pagination
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$limit = 12; // Members per page
$offset = ($page - 1) * $limit;

try {
    $db = Database::getInstance()->getConnection();
    
    // Get library information with address and email
    ...
    // Get total member count for this library
    $countStmt = $db->prepare("SELECT COUNT(*) as total FROM users WHERE library_id = ? AND role = 'member'");
    $countStmt->execute([$libraryId]);
    $totalMembers = $countStmt->fetch()['total'];
    $totalPages = ceil($totalMembers / $limit);
    
    // Get members for this library with pagination
    $stmt = $db->prepare("
        SELECT 
            u.id,
            u.user_id,
    ...
    die('Error loading member data: ' . $e->getMessage());
}
...
<body>
    ...
    <script>
        // Function to style badges for print
        function styleBadgesForPrint() {
            ...
            for (var i = 0; i < badges.length; i++) {
                ...
                for (var j = 0; j < classes.length; j++) {
                    if (classes[j].startsWith('status-')) {
                        status = classes[j].replace('status-', '');
                        break;
                    }
                }
                ...
                switch(status) {
                    case 'active':
                        badge.style.backgroundColor = '#1b5e20'; // Deep green
                        break;
                    case 'inactive':
                    ...
                    case 'pending':
                        ...
                        break;
                    case 'suspended':
                        badge.style.backgroundColor = '#c62828'; // Deep red
                        badge.style.color = 'white';
                        break;
                    default:
                        badge.style.backgroundColor = '#6c757d'; // Default gray
                        badge.style.color = 'white';
                }
            }
        }
        ...
        // Also call it immediately in case of direct print
        window.addEventListener('load', function() {
            // Only apply if we're in print mode
    ...
    <div class="container">
        <div class="page-header">
            ...
            <p>Members of <?php echo htmlspecialchars($library['library_name']); ?></p>
        </div>
        ...
        <div class="content-card print-area">
            <div class="card-header">
                ...
                <div class="btn-group no-print">
                    <a href="member_reports.php" class="back-btn">
            ...
            <div class="library-info">
                ...
                <div class="library-details">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value"><?php echo $totalMembers; ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="members-section">
                ...
                <?php if (empty($members)): ?>
                    <div class="no-data">
                        <i class="fas fa-user fa-3x" style="margin-bottom: 1rem; color: #ced4da;"></i>
                        <h3>No Members Found</h3>
                        <p>This library currently has no members.</p>
                    </div>
                <?php else: ?>
