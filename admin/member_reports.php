...
// Get libraries data with search functionality
try {
    $db = Database::getInstance()->getConnection();
    
    // Get all libraries with member counts, with search functionality
    if (!empty($search)) {
        $stmt = $db->prepare("
            SELECT 
                l.id,
            ...
            FROM libraries l
            LEFT JOIN users u ON l.id = u.library_id AND u.role = 'member'
...
<head>
    ...
    <style>
        ...
        .back-btn:hover {
            background: var(--primary-dark); /* Changed to primary dark */
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(0, 102, 204, 0.3);
        }
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-user-graduate"></i> <?php echo $pageTitle; ?></h1>
            <p>View member details for each library</p>
        </div>
        ...
        <div class="content-card">
                ...
                <form method="GET" style="display: contents;">
                    <div class="search-container">
                        <div class="search-group">
                            ...
                            <?php if (!empty($search)): ?>
                                <a href="member_reports.php" class="btn btn-outline">
            ...
            <div class="table-container">
                <?php if (empty($libraries)): ?>
                    <div class="no-data">
                        <i class="fas fa-inbox fa-3x" style="margin-bottom: 1rem; color: #ced4da;"></i>
                        <h3>No Libraries Found</h3>
                        <p>There are currently no libraries in the system.</p>
                    </div>
                <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="member-count">
                                            <?php echo $library['member_count']; ?>
                                        </span>
                                    </td>
                                    <td>
                                        <a href="view_library_members.php?library_id=<?php echo $library['id']; ?>" 
                                           class="action-btn" 
                                           title="View Members">
