...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_image'])) {
    ...
    if (empty($_FILES['profile_image']['name'])) {
        ...
        $_SESSION['toast_message'] = 'Please select an image file to upload.';
...
<head>
    ...
    <style>
        body {
            background: #f8f9fa; /* Simplified background to match other admin pages */
            min-height: 100vh;
            margin: 0;
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            color: #495057;
            padding-top: 60px; /* Space for navbar - matching other admin pages */
        }
        ...
        .toast.success {
            background: linear-gradient(135deg, #1b5e20 0%, #1b5e20 100%); /* Deep green for success - using user preference */
        }

        .toast.error {
            background: linear-gradient(135deg, #c62828 0%, #c62828 100%); /* Deep red for errors - using user preference */
        }

        .toast.info {
            background: linear-gradient(135deg, #0066cc 0%, #0066cc 100%); /* Deep blue for info - using user preference */
        }

        .toast.warning {
            background: linear-gradient(135deg, #f39c12 0%, #d35400 100%); /* Orange for warnings */
        }
        ...
        .container {
            max-width: 1200px;
            margin: 0 auto;
            padding: 20px; /* Matching other admin pages */
        }

        .page-header {
            margin-bottom: 30px; /* Matching other admin pages */
            text-align: center;
        }

        .page-header h1 {
            color: #2c3e50; /* Matching other admin pages */
            font-size: 1.8rem;
            margin-bottom: 10px;
        }

        .page-header p {
            color: #6c757d; /* Matching other admin pages */
            font-size: 1.1rem;
        }
        ...
        .profile-avatar {
            ...
            border-radius: 50%;
            margin: 0 auto 1rem;
            background: linear-gradient(135deg, #0066cc 0%, #0052a3 100%); /* Using admin blue color */
            display: flex;
            align-items: center;
        ...
        .form-control:focus {
            outline: none;
            border-color: #0066cc; /* Using admin blue color */
        ...
        <div class="profile-container">
            <div class="profile-sidebar">
                ...
                <?php if (isset($_SESSION['profile_image']) && !empty($_SESSION['profile_image'])): ?>
                    <form method="POST" style="margin-top: 10px;">
                        <button type="submit" name="remove_profile_image" class="btn btn-danger" style="width: 100%;" 
            ...
            <div class="content-card">
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        <label>Role</label>
                        <div class="form-control-static">Administrator</div>
                    </div>
