...
$userRole = isset($_SESSION['user_role']) ? $_SESSION['user_role'] : 'member';
...
if (isset($_SESSION['user_id'])) {
    try {
        ...
        $stmt->execute([$user_string_id ?? $user['user_id'] ?? $_SESSION['user_id']]);
...
<style>
    /* Member Navbar Styles - Embedded CSS */
    .member-navbar {
    ...
    .navbar-brand a {
        ...
        font-weight: 700;
        color: #2980b9; /* Blue color for brand text */
    ...
    .navbar-brand i {
        color: #2980b9; /* Blue color for icon */
    ...
    .user-avatar {
        width: 32px; /* Reduced avatar size */
        height: 32px; /* Reduced avatar size */
        border-radius: 50%;
        overflow: hidden;
        display: flex;
        align-items: center;
    ...
    @keyframes fadeIn {
        from { opacity: 0; transform: translateY(-5px); } /* Reduced animation */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateNotificationBadges() {
        // Only update if we're on a member page
