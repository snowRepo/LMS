...
<body>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="notification-details">
                    <div class="notification-icon-large" id="notificationModalIcon">
                        <i class="fas fa-info-circle"></i>
                    </div>
                    <h3 class="notification-title" id="notificationModalTitle">Notification Title</h3>
                    <div class="notification-meta">
                        <span class="notification-time" id="notificationModalTime">Just now</span>
                        <span class="notification-type" id="notificationModalType">Info</span>
                    </div>
                    <div class="notification-message-full">
                        <p id="notificationModalMessage">Notification message content goes here...</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function showNotificationDetails(notificationDataId) {
            ...
            const timeElement = document.getElementById('notificationModalTime');
        ...
        function markAsRead(notificationId) {
            ...
            // Update the notification in the allNotifications array
            ...
            updateNavbarNotificationCount(unreadCount);
            ...
            .then(data => {
                if (!data.success) {
                    console.error('Server reported error marking as read:', data.error);
                    // Revert the UI if the server reports an error
                    if (notificationItem) notificationItem.classList.add('unread');
                    // Update navbar count again if there was an error
                    const newUnreadCount = allNotifications.filter(n => n.read_at === null).length;
                    updateNavbarNotificationCount(newUnreadCount);
                } else {
                    // Update with server's unread count to stay in sync
                    if (typeof data.unread_count !== 'undefined') {
                        updateNavbarNotificationCount(data.unread_count);
                    }
                }
            })
            .catch(error => {
        ...
        function updateNavbarNotificationCount(count) {
            ...
            let notificationBadge = document.querySelector('.nav-link[href*="notifications.php"] .notification-badge');
            ...
            // Also update the unread count in the notifications page header if we're on that page
            const pageUnreadCount = document.querySelector('.notification-count');
            if (pageUnreadCount) {
                pageUnreadCount.textContent = `Showing ${allNotifications.length} notifications (${count} unread)`;
            }
        }
    </script>
</body>
